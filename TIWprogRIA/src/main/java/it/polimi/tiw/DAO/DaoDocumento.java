...
public class DaoDocumento {
	...
	public DaoDocumento(Connection connesione) {
		...
		try (PreparedStatement pstatement = connesione.prepareStatement(query);) {
			...
			try (ResultSet result = pstatement.executeQuery();) {
				if (!result.isBeforeFirst()) // no results, credential check failed
					...
					file.setSommario(result.getString("sommario"));
	...
	public void cancella(int fileId) throws SQLException {
		String query = "DELETE FROM  documento WHERE idDocumento=? ";
		try (PreparedStatement pstatement = connesione.prepareStatement(query);) {
			pstatement.setInt(1, fileId);
	...
	public boolean esisteInCartella(String Name , int subCartellaId) throws SQLException {
		String query = "SELECT * FROM documento WHERE documento.fileName=? AND documento.idSubCartella =?";
