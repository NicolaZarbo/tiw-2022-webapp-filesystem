...
private Connection connesione;
		...
		try (PreparedStatement pstatement = connesione.prepareStatement(query);) {
			...
			try (ResultSet result = pstatement.executeQuery();) {
				if (!result.isBeforeFirst()) // no results, credential check failed
					...
					DaoDocumento fDao=new DaoDocumento(connesione);
					...
					folder.setNome(result.getString("nome"));
					if(files!=null)
	...
	public void creaSottoCartella(int cartellaId,String nome) throws SQLException {
		String query = "INSERT into  subcartella ( idCartella,  dataCreazione, nome) VALUES (?,?,?) ";
		try (PreparedStatement pstatement = connesione.prepareStatement(query);) {
			pstatement.setDate(2, new java.sql.Date(Calendar.getInstance().getTimeInMillis()));
			pstatement.setInt(1, cartellaId);
			...
			try (ResultSet result = pstatement.executeQuery();) {
				if (!result.isBeforeFirst()) // no results, credential check failed
					...
					while(result.next()) {
						...
						sub.setNome(result.getString("nome"));
